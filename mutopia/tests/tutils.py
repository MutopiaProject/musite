...
def load_some_composers():
    Composer.objects.get_or_create(composer='MouseM',
                                   description='Mickey Mouse (1928--)')
    Composer.objects.get_or_create(composer='OylO',
                                   description='Olive Oyl (1928--)')


def load_some_instruments():
    Instrument.objects.get_or_create(instrument='Uke',
                                     in_mutopia=True)
    Instrument.objects.get_or_create(instrument='Banjo',
                                     in_mutopia=True)
...
def make_piece(piece_id=1, title='Flimmin on the Jimjam'):
    ...
    m,_ = Contributor.objects.get_or_create(name='John Smith',
                                            ...
                                            url='johnsmith.com')
    cc,_ = License.objects.get_or_create(name='Public',
                                         url='http://public-domain/')
    ...
    p.save()
    piano, _ = Instrument.objects.get_or_create(instrument='Piano',
                                                in_mutopia=True)
